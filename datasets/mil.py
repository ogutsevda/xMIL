...
class MILFeatureDataset(Dataset):
    ...
    def __init__(self, split_path, metadata_dirs, subsets, patches_dirs, features_dirs, label_cols,
                 bag_size=None, patch_filters=None, preload_features=False, drop_duplicates='sample',
                 max_bag_size=None):
        super(MILFeatureDataset, self).__init__()
        # Save args
        ...
        if max_bag_size is not None and max_bag_size > 0:
            keep_slides = [idx for idx in self.split_metadata.index if len(self.patch_ids[idx]) <= max_bag_size]
    ...
    def bag_collate_fn(batch_list):
        ...
        for key in batch_list[0].keys():
            if key in ['features', 'patch_ids']:
                ...
                col_batch[key] = [batch[key] for batch in batch_list]
            elif key == 'targets':
    ...
    def __getitem__(self, idx):
        ...
        return {'features': features, 'bag_size': len(features), 'targets': targets, 'patch_ids': patch_ids,
                'source_id': source_id, 'slide_id': slide_id}
