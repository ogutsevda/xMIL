...
- A metadata folder containing
  - a file "case_metadata.csv" with one row per case and columns for the "case_id" and the prediction target, and
  - a file "slide_metadata.csv" with one row per slide and columns for the "case_id" and the "slide_id".
- A patch folder containing a folder per slide with patch files and a "metadata/df.csv" file with one row per patch and a column "patch_id".
- A feature folder containing a PyTorch file "{slide_id}.pt" per slide, which includes a Tensor of extracted features in the same order as the sorted "patch_id" values (ascending) of this slide. The shape of each Tensor should be (num_patches, num_features).
