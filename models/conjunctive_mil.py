...
def get_conjunctive_mil_model(input_dim=1024, num_classes=2, hidden_dim=256, device='cpu'):
    model = DefaultMILGraph(
        ...
        classifier=ConjunctiveClassifier(
...
class ConjunctiveClassifier(torch.nn.Module):
    ...
    def forward(self, features, attention, bag_sizes):
        ...
        weighted_patch_logits = attention * patch_logits
        bag_logits = self.aggregate_patch_scores(weighted_patch_logits, bag_sizes)
        classifier_out_dict = {}
        classifier_out_dict['logits'] = bag_logits
        classifier_out_dict['patch_logits'] = weighted_patch_logits
        return classifier_out_dict


class xConjunctiveMIL(xMIL):
